...
Picking a template pre-fills the name, instruction, and schedule so you can adjust before saving, rather than writing it from a blank form. Templates that place phone calls or send WhatsApp messages need those [channels](/interfaces/whatsapp) linked first; if they aren't, the template falls back to email where that makes sense (Feature Tips does this automatically).
