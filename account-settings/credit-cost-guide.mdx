...
Every response costs credits based on two things: which model handled it, and how much it actually had to read and write. There's no flat per-message fee. A one-line answer on a light model can cost a single credit; a long research task on a premium model can cost dozens. See [What uses credits](/do-work-with-it/what-uses-credits) for when spend actually starts and how to keep it down; this page is the numbers.
